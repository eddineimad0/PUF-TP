...
bool b64_decode(const void* data, const uint32_t len, uint8_t* output, const uint32_t output_len, uint32_t* count){
    *count = 0;
    ...
    /*     return false; */
    /* } */
    mem_set(output, 0x00, output_len);

    for (uint32_t i = 0; i < last_index; i += 4)
    {
        uint32_t n = (uint32_t) B64index[p[i]] << 18 | (uint32_t)B64index[p[i + 1]] << 12 | (uint32_t)B64index[p[i + 2]] << 6 | (uint32_t)B64index[p[i + 3]];
    ...
    if (pad1)
    {
        uint32_t n = (uint32_t)B64index[p[last_index]] << 18 | (uint32_t)B64index[p[last_index + 1]] << 12;
        output[j++] = (uint8_t)(n >> 16);

        if (pad2)
        {
            n |= (uint32_t)B64index[p[last_index + 2]] << 6;
            output[j++] = (uint8_t)(n >> 8 & 0xFF);
        }
    }
